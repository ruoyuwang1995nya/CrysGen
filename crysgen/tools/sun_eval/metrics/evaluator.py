...
class MetricsEvaluator:
    ...
    def compute_metrics(
        ...
        print(f"Computing metrics: {metrics_classes}")
        # pre_aggregate values for metrics that support it
        metrics_pre_aggregete={}
        for metric_cls in metrics_classes:
            ...
            if calc_pre_aggregate and hasattr(metric, "pre_aggregation_values"):
                metrics_pre_aggregete[metric.name] = metric.pre_aggregation_values
        if pretty_print:
            logger.info(
                json.dumps(
                    {
        ...
        results={k: {"value":v["value"]} for k, v in metrics_dict.items()}
        if calc_pre_aggregate:
            for k, v in metrics_pre_aggregete.items():
                results[k].update(
                    {"pre_aggregation_values": v}
            )
        return results
